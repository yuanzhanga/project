<template>
  <div class="content">
    <div class="header">
      ...
      <el-button type="primary" @click="newBtnClick" v-if="isCreate">新建数据</el-button>
    </div>
    <div class="list">
...
const { postUsersListAction, deleteUserAction } = systemStore
const { postPageListAction } = systemStore
const { userList,pageTotalCount,pageList, usersTotalCount } = storeToRefs(systemStore)
//判断是否有权限
...
fetchUserListData() //第一次展示页面进行一次网络请求
...
//#region获取列表数据
function fetchUserListData(queryInfo = {}) {
  ...
  postPageListAction('users',info)
}
//#endregion
defineExpose({ fetchUserListData }) //暴露网络请求给父组件
</script>
